...
describe 'working through various arrays' do
  ...
  context 'test 2 dimensional arrays' do
    ...
    expect(dog_breed_and_names[0]).to eq ['jack russell', 'archie']
    end

    it 'the second last entry should be a bloodhound called droopy' do
      expect(dog_breed_and_names.last).to eq ['greyhound', 'streak']
    end

    it 'the second entry should be a boxer called bruce' do
      expect(dog_breed_and_names[1]).to eq ['boxer', 'bruce']
    end
  ...
  context 'test multidimensional arrays' do
    ...
    it 'the last dog should be called streak and is an old dog' do
      expect(dog_breed_names_age_and_health.last).to eq ['greyhound', 'streak',[12, 'old dog']]
    end

    it 'the bloodhound should be 6 years old and healthy' do
      expect(dog_breed_names_age_and_health[2]).to eq ['bloodhound', 'droopy', [6, 'healthy']]
    end

    it 'the first dog should be a jack russell and 4 years old' do
      expect(dog_breed_names_age_and_health[0][2][0]).to eq 4
      expect(dog_breed_names_age_and_health[0][0]s).to eq 'jack russell'
